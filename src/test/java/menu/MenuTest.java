...
public class MenuTest {
	...
	public static List<MenuResponse> menuTree(List<SystemMenu> menus){
		...
		for(SystemMenu systemMenu:menus){
			if(systemMenu.getParentId()==0){
				level = 1;
				MenuResponse response = beanToResponse(systemMenu,level);
				response.setChildren(setChildMenu(systemMenu.getId(),menus,level));
	...
	public static List<MenuResponse> setChildMenu(int id,List<SystemMenu> menus,int level){
		level ++;
	...
	public static MenuResponse beanToResponse(SystemMenu systemMenu,int level){
